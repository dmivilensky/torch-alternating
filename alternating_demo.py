...
from alternating import AlternatingEnvelopeAccelerated, AlternatingEnvelopeStandard, SwitchingStrategy
...
def closure(backward=True):
    ...
    # for batch in []:
    #     y_pred = model(X_train)
    #     loss = loss_fn(y_pred, y_train) / batches_in_dataset
    #     value += loss.item()
    #     if backward:
    #         loss.backward()
    return value
...
if ALTERNATING_ALGORITHM == AlternatingAlgorithm.STANDARD:
    alternation = AlternatingEnvelopeStandard(
        chain(model.layer1.parameters(), model.layer2.parameters()),
...
for epoch in range(EPOCHS):
    ...
    if (epoch + 1) == ((EPOCHS - 1) // ALTERNATING_PERIOD) * ALTERNATING_PERIOD:
        alternation.takeoff()
        blank_shooting = True
    
    alternation.step(closure=closure, blank=blank_shooting)
